...
Index API在一個特定的索引，新增或更新一個已轉化的JSON文件，使其變得可以搜尋。 以下範例插入一個JSON文件到"twitter"索引，後方有一個名為"tweet"的類型及值為1的id:
...
如果索引事先沒有被建立(請參閱手動建立索引那章)，索引操作會自動建立一個索引；此外，若其中一個類型沒被建立mapping(請參閱手動建立mapping那章)，還會為該類型自動建立一個動態的類型mapping。

mapping本身是非常彈性，且是無架構的。新的欄位和物件會被自動加到指定類型的mapping定義。 請參閱mapping章節，以取得更mapping定義的資訊。

注意! 如果設定`index.mapping.allow_type_wrapper`為`true`，JSON文件的格式也可以包含類型(在使用JSON mapper的時候很方便)，舉例來說:
...
在所有nodes的config檔案，自動建立索引是可以藉由將`action.auto_create_index`設為`false`而關閉。
在所有nodes的config檔案(或指定索引的設定)，自動建立mapping是可以藉由將`index.mapper.dynamic`設為`false`而關閉。

自動建立索引可以包含一個基於white/black清單的pattern，舉例來說: 將`action.auto_create_index`設為`+aaa*,-bbb*,+ccc*,-*`(+為允許，-為禁止)。
